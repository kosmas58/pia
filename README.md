...
Since Angular easily supports building Progressive Web Apps (PWA), I extended PIA to be installed as PWA.

Run `yarn pwa:create-pwa-icons` creates the necessary icons based on `icon.png/` stored in the root directory.  
Run `yarn pwa:start-server` starts a http-server running from `dist/` created by `yarn prod` to test the PWA.

You can install PIA as PWA from the chrome settings menu choosing `Install PIA`.  
From the app menu of chrome, you are able to uninstall PIA as PWA.
