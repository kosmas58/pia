...
export class PiaService extends ApplicationDb {
   ...
   */
  async importData(data: any, prefix: string, is_duplicate: boolean, is_example?: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      if (!data.pia) {
  ...
  private async destroyData(piaId: number) {
    ...
    await this.evaluationService.findAllByPia(piaId).then(async (response: Array<Comment>) => {
      for (const c of response) {
