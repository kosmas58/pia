...
export class KnowledgeBaseService extends ApplicationDb {
  ...
  constructor(private router: Router, public translateService: TranslateService, private knowledgesService: KnowledgesService) {
  ...
  duplicate(id: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.find(id).then((data: KnowledgeBase) => {
        this.create(new KnowledgeBase(null, data.name + ' (copy)', data.author, data.contributors))
