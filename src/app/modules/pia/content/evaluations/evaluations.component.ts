...
export class EvaluationsComponent implements OnInit, AfterViewChecked, OnDestroy, DoCheck {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.riskName = {
      value: this.translateService.instant('sections.3.items.' + this.item.id + '.title')
    };

    // Updating translations when changing language (risks' names)
    this.riskSubscription = this.translateService.onLangChange.subscribe((event: LangChangeEvent) => {
      this.riskName = {
        value: this.translateService.instant('sections.3.items.' + this.item.id + '.title')
      };
    });
    ...
    this.placeholderSubscription = this.translateService.onLangChange.subscribe((event: LangChangeEvent) => {
      if (this.evaluation && this.evaluation.status) {
        if (this.evaluation.status === 1) {
          this.comment_placeholder = this.translateService.instant('evaluations.placeholder_to_correct');
        } else if (this.evaluation.status === 3) {
          this.comment_placeholder = this.translateService.instant('evaluations.placeholder_acceptable');
        } else {
          this.comment_placeholder = this.translateService.instant('evaluations.placeholder_improvable2');
        }
      }
    });
  }
  ...
  actionPlanCommentFocusIn(): void {
    if (this.globalEvaluationService.evaluationEditionEnabled) {
      this.knowledgeBaseService.placeholder = this.translateService.instant('evaluations.placeholder_improvable1');
