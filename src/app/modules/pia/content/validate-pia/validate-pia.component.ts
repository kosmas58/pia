...
export class ValidatePIAComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.validateForm.controls['validateStatus1'].patchValue(this.pia.status > 1);
    this.validateForm.controls['validateStatus2'].patchValue(this.pia.status > 1);
  ...
  submitRemoveAttachment() {
    this.attachmentsService.removeAttachment(this.removeAttachmentId, this.removeAttachmentForm.controls['comment'].value).then(() => {
