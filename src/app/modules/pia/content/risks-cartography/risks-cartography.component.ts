...
export class RisksCartographyComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    this.risk1Letter = this.translateService.instant('cartography.risk1_access');
    this.risk2Letter = this.translateService.instant('cartography.risk2_modification');
  ...
  loadCartography() {
    ...
    if (this.dataJSON['risk-disappearance']['author'].x && this.dataJSON['risk-disappearance']['author'].y) {
      ...
      context.fillStyle = '#333';
      try {
        context.fillText('(D)', this.dataJSON['risk-disappearance']['author'].x - 8, this.dataJSON['risk-disappearance']['author'].y + 20);
      } catch (ex) {}
    }
