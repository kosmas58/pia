...
export class RevisionPreviewComponent implements OnInit {
            ...
            if (answer) {
              ...
              if (answerModel.data) {
                ...
                if (answerModel.data.gauge && answerModel.data.gauge > 0) {
                  content.push(this.translateService.instant(this.piaService.getGaugeName(answerModel.data.gauge)));
                }
  ...
  public exportJson(): void {
    const revisionDate = this.datePipe.transform(this.revision.created_at, '-yyyy-MM-dd-HH-mm');
  ...
  public restoreRevision(): void {
    this.dialogService.confirmThis(
      ...
      () => {
        this.revisionService.loadRevision(this.revision.id).then((piaExport: any) => {
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
            this.router.navigate(['pia', piaExport.pia.id, 'section', 1, 'item', 1]);
          });
        });
      },
      () => {
